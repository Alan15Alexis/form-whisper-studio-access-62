    ...
    duration: 0.5
      ...
      <Card className="shadow-lg border-2 border-primary/20">
        <CardHeader className="text-center pb-4">
          ...
          {formTitle && <CardDescription className="text-base">
              {formTitle}
            </CardDescription>}
        </CardHeader>
        ...
        <CardContent className="space-y-6 py-6">
          <div className="p-6 bg-primary/5 rounded-lg space-y-4 border border-primary/20">
            ...
            {scoreRanges && scoreRanges.length > 0 && !hasValidRanges && <Alert className="mt-4 border-amber-200 bg-amber-50">
            ...
            {isLoading ? <div className="mt-6 p-4 bg-background rounded border text-center">
              ...
              </div> : scoreFeedback ? <div className="mt-6 p-6 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg border-2 border-green-200 text-center">
                <div className="flex items-center justify-center mb-3">
                  <CheckCircle className="h-5 w-5 text-green-600 mr-2" />
                  <h4 className="text-lg font-semibold text-green-800">Tu Resultado</h4>
                </div>
                ...
                <div className="flex items-center justify-center space-x-2 mt-3">
                  ...
                  {feedbackSource && <div className="text-xs text-blue-600 bg-blue-100 rounded-full px-2 py-1">
                      ✅ Desde {feedbackSource}
                    </div>}
                </div>
              </div> : <div className="mt-6 p-4 bg-gray-50 rounded border text-center">
                <div className="text-gray-600">
                  <p className="font-medium">Puntuación registrada: {currentScore} puntos</p>
                  <p className="text-sm text-muted-foreground mt-2">
                    {scoreRanges && scoreRanges.length > 0 ? "No hay mensaje configurado para esta puntuación específica" : "No hay mensajes personalizados configurados para este formulario"}
                  </p>
          ...
          <div className="text-center text-muted-foreground">
            ...
            {formId && <p className="text-xs mt-1">ID del formulario: {formId}</p>}
            {scoreRanges && scoreRanges.length > 0 && <p className="text-xs mt-1">Rangos configurados: {scoreRanges.length}</p>}
          </div>
          
          {/* Next button in bottom right */}
          <div className="flex justify-end mt-8">
            <Button onClick={onNext} className="px-8" disabled={isLoading}>
              Continuar
            </Button>
          </div>
        </CardContent>
